...
@Configuration
public class DefaultDeadExchangeConfig {

    @Value("${rabbit.default.queue.dead}")
    ...
    @Bean
    public AmqpTemplate defaultQueueDead(ConnectionFactory connectionFactory, MessageConverter messageConverter){
    ...
    @PostConstruct
    public void init(){
        amqpAdmin.declareQueue(createQueueDead());
    }
}
